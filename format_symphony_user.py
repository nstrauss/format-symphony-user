...
create and update users with a load users report.

Option to upload resulting text file to SFTP server with '--sftp/-s' flag.
...
10 runs will be kept in generated_ascii directory. Older files are deleted."""
...
def get_grad_year(year, grade):
    ...
    # Exception for non-numeric grades like kindergarten
    try:
        grade = int(grade)
    except ValueError:
        grade = 0
...
def upload_ftp_file(ascii_file):
    ...
    try:
        srv = pysftp.Connection(host=server, port=port, username=username, password=password, cnopts=cnopts)
...
def main():
    ...
    parser.add_argument("-sftp", "--sftp", action="store_true", help="Upload resulting ASCII user data via SFTP")
    ...
    with open(CSV_PATH) as csv_file:
        ...
        for row in reader:
            ...
            for field, value in list(ascii_record.items()):
    ...
    sorted_r = sorted(list(r.items()), reverse=True, key=operator.itemgetter(1))
